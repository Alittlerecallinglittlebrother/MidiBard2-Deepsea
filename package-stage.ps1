...
$pluginZip = Join-Path $OutputDirectory 'MidiBard2-Deepsea-3.2.5.16.zip'
$sourceZip = Join-Path $OutputDirectory 'MidiBard2-Deepsea-3.2.5.16-source.zip'
$screenshotsZip = Join-Path $OutputDirectory 'MidiBard2-Deepsea-3.2.5.16-screenshots.zip'
...
if ($manifest.InternalName -ne 'MidiBard2' -or $version -ne '3.2.5.16' -or $manifest.AssemblyVersion -ne $version -or $manifest.DalamudApiLevel -ne 15) { throw 'Unexpected plugin manifest.' }
if ($manifest.Name -cne 'midibard2-深海回响特供版' -or $manifest.Author -cne 'akira0245, Ori, Kalle, Zune, 断水剑') { throw 'Unexpected plugin branding.' }
...
try {
    ...
    foreach ($name in @('LICENSE', 'LIVE-INSTRUMENT-SWITCH.md', 'STAGE-README.md', 'STAGE-ROOM.md', 'STAGE-IPC.md', 'STAGE-VERIFICATION.md')) { Add-ZipFile $archive (Join-Path $PSScriptRoot $name) $name }
...
try {
    Get-ChildItem -LiteralPath $PSScriptRoot -Recurse -File -Force | ForEach-Object {
        ...
        if ($relative -notmatch '(^|/)(\.git|bin|obj|verification|artifacts)(/|$)' -and $_.Extension -ne '.log') { Add-ZipFile $archive $_.FullName ('MidiBard2-Deepsea-v0.12/' + $relative) }
    }
}
finally { $archive.Dispose() }
...
$hashes | Set-Content -LiteralPath (Join-Path $OutputDirectory 'MidiBard2-Deepsea-3.2.5.16-SHA256SUMS.txt') -Encoding utf8
$hashes
