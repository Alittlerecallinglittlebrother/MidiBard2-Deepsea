...
{
    [IPCHandle(MessageTypeCode.StageTransport)]
    private static void HandleStageTransport(IPCEnvelope message)
    {
        _ = api.Framework.RunOnFrameworkThread(() =>
        {
            if (!api.ClientState.IsLoggedIn || message.PartyId != api.PartyList.PartyId
                || message.BroadcasterId != (long?)(api.PartyList.GetPartyLeader()?.ContentId)) return;
            StageIntegration.EnsembleTransport.Receive(message.StringData);
        });
    }
    ...
    public static void LoadPlayback(int index, bool includeSelf = false)
    {
        ...
        new IPCEnvelope(MessageTypeCode.LoadPlaybackIndex, index.ToBytesUnmanaged(), order).BroadCast();
    }
    ...
    private static void HandleSetInstrument(IPCEnvelope message)
    {
        ...
        if (MidiBard.CurrentPlayback.MidiFileConfig.AutomaticallyAssigned)
        {
            SwitchInstrument.SwitchToContinue(MidiBard.CurrentPlayback.GetInstrumentId());
            return;
        }
