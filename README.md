...
**3.2.5.24** 新增跨电脑当前歌曲同步、队长手动分配后一键下发，以及跟随队长、队形编辑与统一列队。歌曲同步默认关闭，移动接收每次加载后默认关闭。使用步骤见 [歌曲同步](LOCAL-SONG-SYNC.md)、[手动分配](LOCAL-MANUAL-ASSIGNMENT.md) 和 [移动与队形](LOCAL-MOVEMENT.md)。

在线版本 **3.2.5.24**，Dalamud API 15，Windows x64 / .NET 10。作者署名：akira0245, Ori, Kalle, Zune, 断水剑, SevenCat。

基于midibard2的魔改版本，添加自动点歌、歌单、控制台、不同IP连接功能。

本插件基于midibard2开源代码魔改制作，完全免费，旨在打造一个更低门槛、更有活力的游戏演奏环境

已安装旧版的用户，刷新卫月插件仓库后可直接更新至 **3.2.5.24**。使用本地测试版的用户请先停用开发插件，再启用在线版本；若继续使用开发插件，请完整解压 [3.2.5.24 安装包](https://github.com/Alittlerecallinglittlebrother/MidiBard2-Deepsea/releases/download/v3.2.5.24/MidiBard2.zip)，重新指定其中的 `MidiBard2.dll`。不要同时启用多个版本。全队与主持人都应升级。
...
- 跨电脑只需队长准备 MIDI，缺歌的队员自动接收当前歌曲至独立缓存，个人曲库和排序保持不变。
- 队长可关闭自动分配，在原合奏面板手动指定每轨演奏人、乐器和移调，再统一下发；原自动模式保留。
- 队长可带队地面跟随，停下后编辑、保存并统一下发乐手站位与朝向；每端须主动开启移动接收。
- 主持人无需入队，可通过共享房间查看和调整队列；普通队员使用查看邀请。
- 转让游戏队长时接管演奏权限，保留房间、主持人连接和队列。原房主仍须在线、留在同一小队并保持樱花隧道运行。
- 节目单、演出记录及导出；主窗口和自动点歌窗口提供“联系作者”按钮。

跨电脑合奏须开启 PMD、合奏监听和当前歌曲同步，全队加入同一演出房间；公网连接沿用已有 TCP 隧道。关闭歌曲同步时，各端仍需自行准备相同 MIDI。候任队长须提前加入原房间。详见 [使用说明](STAGE-README.md)、[演出房间与自动接管](STAGE-ROOM.md) 和 [状态接口](STAGE-IPC.md)。

![自动点歌与联系作者入口](docs/images/queue.png)

## 3.2.5.24 更新

整合此前本地版的歌曲分发、手动分配，以及本版移动与队形。歌曲下发等待全员载入成功后才允许插件发起合奏准备；手动方案绑定歌曲内容与当前小队。移动支持跟随、站位编辑、朝向、预设及停止，按角色身份分配站位，不受各端小队显示顺序影响。

移动接收每次加载或换角色后默认关闭；按移动键或 Esc、断线、切图、开始演奏等情况会停止。仅支持同场景地面移动，不含绕障寻路、骑乘、飞行、游泳或跨图导航。保留原演奏流程，不含演奏中换乐器功能。

已完成编译、核心、回环 TLS、界面及静态签名检查；真实多机 FF14、公网演奏和移动验收仍待完成。检查范围见 [验证记录](STAGE-VERIFICATION.md)。

## 3.2.5.21 更新

修复异常 MIDI 触发共享曲库每秒重复导入，导致窗口闪烁、搜索输入被反复禁用的问题。曲库导入对通道事件参数采用与播放器相同的 `ReadValid` 容错策略，仅作用于内存，不改写 MIDI 原文件。仍无法读取的文件在没有变化时跳过，修复文件或点击“刷新共享曲库”后重试；保存失败单独记录，不再作为同步成功。

曲库搜索、人数筛选、选择及查看歌曲在后台同步期间保持可用；写入、删除和加入队列继续遵守原有权限与忙碌保护。本版保留原有配置与 MIDI 文件，可通过在线仓库更新。验证见 [验证记录](STAGE-VERIFICATION.md)。

## 3.2.5.20 更新

修复自动点歌窗口顶部开源免费说明换行的问题，使整句始终完整显示在同一行。空间不足时仅缩小此说明字体，不再逐字换行；说明内容、其他控件与演奏行为保持不变，作者列表保留 SevenCat。

演奏逻辑沿用 3.2.5.17，不含实验性演奏中换乐器功能。验证范围及实际结果见 [验证记录](STAGE-VERIFICATION.md)。
